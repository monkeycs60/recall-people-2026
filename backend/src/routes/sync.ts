...
const contactPayloadSchema = z.object({
  ...
  loves: z.array(z.string()).nullable().optional(),
...
async function encryptContactPayload(encryptionKey: string | undefined, userId: string, payload: ContactPayload) {
  return {
    ...
    encryptedLoves: payload.loves == null ? null : await encryptJson(encryptionKey, payload.loves),
...
async function decryptContact(encryptionKey: string | undefined, row: any) {
  return {
    ...
    loves: row.encryptedLoves ? await decryptJson<string[]>(encryptionKey, row.encryptedLoves) : [],
