# App Store Optimization — iOS (Français)

## Nom de l'app (30 caractères max)
```
Recall People - CRM Personnel
```
**Caractères : 30**

## Sous-titre (30 caractères max)
```
N'oubliez plus vos rencontres
```
**Caractères : 29**

## Mots-clés (100 caractères max, séparés par des virgules)
```
crm personnel,notes vocales,contacts,mémoire,relations,networking,rappels,ia,noms,conversations
```
**Caractères : 96**

## Texte promotionnel (170 caractères max)
*Peut être mis à jour sans review*
```
14 jours d'essai gratuit. Enregistrez une note vocale, l'IA extrait noms et infos. Rappels intelligents pour ne jamais perdre le fil de vos relations.
```
**Caractères : 155**

## Description (4000 caractères max)

```
Vous rencontrez quelqu'un à une conférence. Vous discutez de sa startup, de ses enfants, de son prochain voyage au Japon.

Le lendemain matin ? Trou noir.

Ça vous parle ? Vous n'êtes pas seul. On oublie 80% de chaque conversation en 24 heures.

Recall People règle ça — avec votre voix.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

COMMENT ÇA MARCHE

Parlez, c'est tout. Après n'importe quelle conversation, ouvrez Recall et enregistrez une note vocale :

« Je viens de prendre un café avec Sarah. Elle passe un entretien chez Google la semaine prochaine, et elle déménage enfin dans son nouvel appart à Lyon... »

C'est tout. Recall fait le reste :

✓ Extrait les noms, les infos et les dates automatiquement
✓ Crée ou met à jour les fiches contact
✓ Programme des rappels pour les événements à venir
✓ Génère des résumés IA de chaque personne

Pas de saisie. Pas d'organisation. Juste votre voix.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

FONCTIONNALITÉS

◆ CAPTURE VOCALE
Enregistrez des notes en quelques secondes. Parlez naturellement dans l'une des 5 langues supportées — français, anglais, espagnol, allemand ou italien. L'IA transcrit et extrait les infos instantanément.

◆ FICHES CONTACT ENRICHIES
Chaque détail que vous avez mentionné sur quelqu'un, au même endroit. Métier, famille, hobbies, projets — plus un résumé IA qui vous dit qui est vraiment cette personne.

◆ FIL D'ÉVÉNEMENTS
Ne ratez plus ce qui compte. L'entretien de Sarah ? Le marathon de Marcus ? L'anniversaire d'Emma ? Recall vous envoie un rappel la veille.

◆ RAPPELS INTELLIGENTS
Recall vous prévient quand vous n'avez pas eu de nouvelles d'un contact depuis longtemps. En version Pro : digest hebdomadaire, fréquence personnalisée par contact, et suivi post-événement automatique.

◆ RECHERCHE SÉMANTIQUE
Trouvez n'importe qui avec une question. « Qui travaille dans la tech ? » « Les gens que j'ai rencontrés à la conférence » « Contacts qui ont des enfants » — décrivez simplement ce que vous cherchez.

◆ VIE PRIVÉE PAR DÉFAUT
Vos données restent sur votre appareil. Pas de sync cloud. Pas d'accès tiers. Tout est stocké localement et chiffré.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

POUR QUI ?

• Professionnels qui rencontrent beaucoup de monde
• Ceux qui veulent être plus attentionnés dans leurs relations
• Networkers qui détestent oublier les noms
• Personnes qui accordent de l'importance aux petits détails

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

TARIFS

Essai gratuit : 14 jours d'accès complet
Gratuit : 15 contacts, notes illimitées, 5 avatars IA/mois, 10 questions IA/mois
Pro : 4,99 €/mois ou 44,99 €/an — contacts, avatars et IA illimités, rappels intelligents

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Les petits détails font les grandes différences.

Téléchargez Recall People et n'oubliez plus jamais ce qui compte.
```
**Caractères : ~2,500**

## Nouveautés (4000 caractères max)
```
Nouveau modèle freemium !
• 14 jours d'essai gratuit avec accès complet
• Notes illimitées pour tous
• Rappels intelligents : ne perdez plus le fil de vos relations
• Quotas mensuels renouvelables (avatars IA, questions IA)
• Nouveau tarif Pro : 4,99 €/mois ou 44,99 €/an
```

## Suggestions de textes pour les screenshots

1. **Écran principal** : « N'oubliez plus vos rencontres »
2. **Enregistrement vocal** : « Parlez, c'est tout. »
3. **Fiche contact** : « Tout ce que vous savez. En un coup d'œil. »
4. **Fil d'événements** : « Ne ratez plus ce qui compte »
5. **Recherche** : « Trouvez n'importe qui avec une question »
6. **Rappels** : « Recall vous rappelle de prendre des nouvelles »
